...
test('parses IRC tags, prefix, command, params and trailing text', () => {
  const parsed = parseTwitchIrcLine(
    '@display-name=Ana\\sMaria;id=abc :ana!ana@ana.tmi.twitch.tv PRIVMSG #canal :ola chat',
  );
  ...
  assert.deepEqual(parsed.params, ['#canal']);
  assert.equal(parsed.trailing, 'ola chat');
});

test('converts Twitch PRIVMSG into the canonical chat message model', () => {
  const message = parseTwitchPrivmsg(
    '@badge-info=;badges=;color=#8A2BE2;display-name=Streamer;id=message-1;tmi-sent-ts=1780603200000;user-id=user-1 :streamer!streamer@streamer.tmi.twitch.tv PRIVMSG #canal :mensagem real',
  );
  ...
  assert.equal(message.text, 'mensagem real');
